...
def file_writer(data, daynormal2, HH, MM):
    f = open(out_dir + 'CLDTOPZ_' + daynormal2 + '_' + HH + ':' + MM + ':' + '00.f95', 'w')
...
#    f.write("     OPEN(UNIT=IUNIT, file=" + " 'GR_CTH:" +  daynormal2 + '_' + HH + ':' + MM + ':' + '00' + "', status='new', & \n")
    f.write("     OPEN(UNIT=IUNIT, file=" + "'CLDTOPZ:" +  daynormal2 + '_' + HH  + "', status='new', & \n")
...
#        print(os.path.basename(file))
        year = int(os.path.basename(file)[24:28])
        dayjulian = int(os.path.basename(file)[28:31]) - 1
        daynormal = datetime.datetime(year,1,1) + datetime.timedelta(dayjulian)
        daynormal2 = daynormal.strftime('%Y-%m-%d')
        HH = os.path.basename(file)[31:33] 
        MM = os.path.basename(file)[33:35]
#        print(year)
#        print(dayjulian)
#        print(daynormal2)
#        print(HH)
...
if __name__ == '__main__':
    ...
    for i in range(0,4):
        ...
        nc_folder = '/home/alton/Satellie_Imagery/Data/CTOPH/20200622/' + hour + '/'
        out_dir = '/home/alton/Github/MADWRF-Development/Satellite_Reprojection_scripts/WPS_Intermediate_Files/CTOPH/'
        files = glob(nc_folder+'OR_ABI-L2-ACHAF*.nc')
