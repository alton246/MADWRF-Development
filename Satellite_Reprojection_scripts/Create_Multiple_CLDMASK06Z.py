...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 13 18:44:29 2021
...
def file_writer(data, daynormal2, HH, MM):
    ...
    f.write("  character(len=9) :: FIELD= 'CLDMASK'\n" )
    f.write("  character(len=25) :: UNITS='Unknown' \n")
    f.write("  character(len=46) :: DESC='1-Cloud 0-No Cloud' \n")
...
##       f.write('  real, allocatable, dimension(' + str(data['real_lon'].shape[1]) + ',' +  str(data['real_lat'].shape[0]) + ')' +' :: SLAB \n')
    ...
    f.write("     OPEN(UNIT=IUNIT, file=" + "'CLDMASK:" +  daynormal2 + '_' + HH + "', status='new', & \n")
...
#        print(file)
        year = int(os.path.basename(file)[23:27])
        dayjulian = int(os.path.basename(file)[27:30]) - 1
        daynormal = datetime.datetime(year,1,1) + datetime.timedelta(dayjulian)
        daynormal2 = daynormal.strftime('%Y-%m-%d')
        HH = os.path.basename(file)[30:32] 
        MM = os.path.basename(file)[32:34]
...
if __name__ == '__main__':
    ...
    for i in range(16,19):
        ...
        nc_folder = '/home/alton/Satellie_Imagery/Data/Clear_Sky_Mask/20200622/' + hour + '/'
        out_dir = '/home/alton/Github/MADWRF-Development/Satellite_Reprojection_scripts/WPS_Intermediate_Files/CLDMASK/'
        files = glob(nc_folder+'OR_ABI-L2-ACMF*.nc')
