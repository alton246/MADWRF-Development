...
def lat_lon_reproj2(nc_folder, nc_indx):
    ...
    g16_data_file = nc_files[nc_indx] # select .nc file
    print(nc_files[nc_indx])  # print file name
    year = nc_files[nc_indx][27:31]
    
    dayjulian = int(nc_files[nc_indx][31:34]) - 1
    daynormal = datetime.datetime(int(year),1,1) + datetime.timedelta(int(dayjulian))
    daynormal2 = daynormal.strftime('%Y-%m-%d')
    HH = nc_files[nc_indx][34:36] 
    MM = nc_files[nc_indx][36:38]
...
#    DPI = 600
    ...
    cpt = loadCPT('/home/alton/Clouds/Colortables/IR4AVHRR6.cpt')
    ...
    mesh = ax1.pcolormesh(data['real_lon'], data['real_lat'],
                          data['real_data']-273.15, cmap=cpt_convert, vmin=-103, vmax=84)
    ax1.plot(cl[0], cl[1], color='yellow')
    ...
    plt.savefig(png + daynormal2 + '_' + HH + '_' + MM + '_brtemp_2.png',
